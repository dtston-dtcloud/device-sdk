...
/******************************************************************************/
/*
	Open a PEM X.509 certificate array and parse it

	Memory info:
		Caller must free outcert with psX509FreeCert on function success
		Caller does not have to free outcert on function failure
*/
int32 psX509ParseCertArray(psPool_t *pool, char *certarray, int32 certarraylen,
...
*/
		if ((err = pemCertFileBufToX509(pool, certarray, certarraylen, &x509list))
				< PS_SUCCESS) {
			if (firstCert) psX509FreeCert(firstCert);
			return err;
		}

//        psTraceBytes("X509 cert", x509list->item, x509list->len);
/*
...
*/
		if (cert->certAlgorithm == OID_SHA1_RSA_SIG) {
			...
			MD5Init(&hashCtx.md5);
			MD5Update(&hashCtx.md5, certStart, certLen);
			MD5Final(cert->sigHash, &hashCtx.md5);
		}
#endif
#ifdef USE_SHA256
...
*/
		MD5Init(&md5ctx.md5);
		while (len-- > 0) {
			MD5Update(&md5ctx.md5, p, sizeof(char));
			p++;
		}
		MD5Final(oid, &md5ctx.md5);
		noid = lookupExt(oid);
/*
...
static int32 validateDateRange(psX509Cert_t *cert)
{
	...
	rawtime = psGetTime(NULL, NULL);
	icomm_localtime(&rawtime, &tm);
						...
						void *poolUserPtr)
{
	...
	*/
	...
	if (oi == OID_SHA1_RSA_SIG) {
		...
		sigLen = MD5_HASH_SIZE;
		MD5Init(&hashCtx.md5);
		MD5Update(&hashCtx.md5, sigStart, (uint32)(sigEnd - sigStart));
		MD5Final(sigHash, &hashCtx.md5);
#endif
#ifdef USE_SHA256
