...
*/
		...
		memset(desKeyBin, 0x0, DES3_KEY_LEN);
		if ((int32)len != plen) {
			psTraceCrypto("PKCS#8 decryption error\n");
			return PS_FAILURE;
		}
		...
		int32 pLen, unsigned char *macPass, int32 macPassLen)
{
	...
	rc = PS_SUCCESS;
ERR_FBUF:
	memset(iwidePass, 0x0, sizeof(iwidePass));
	memset(mwidePass, 0x0, sizeof(mwidePass));
	psFree(fileBuf, pool);
	return rc;
}
...
int32 pkcs1ParsePrivbuf(psPool_t *pool, char *keyBuf, int keyBufLen, char *password,
			psPubKey_t **outkey)
{
	unsigned char	*DERout;
	int32			rc;
	...
	*outkey = NULL;
	...
	if ((rc = pkcs1DecodePrivbuf(pool, keyBuf, keyBufLen, password, &DERout, &DERlen))
			< PS_SUCCESS) {
		return rc;
	}
...
*/
int32 pkcs1DecodePrivbuf(psPool_t *pool, char *keyBuf, int32 keyBufLen, char *password,
			...
			unsigned char **DERout, uint32 *DERlen)
{
    ...
    rc = pkcs1DecodePrivbuf(pool, keyBuf, keyBufLen, password, DERout, DERlen);
    psFree(keyBuf, pool);
	return rc;
}
				 ...
				 int32 iter, unsigned char *key)
{
	...
	MD5Init(&md.md5);
	MD5Update(&md.md5, pass, passlen);
	MD5Update(&md.md5, salt, 8);
	MD5Final(md5, &md.md5);
	memcpy(key, md5, MD5_HASH_SIZE);

	MD5Init(&md.md5);
	MD5Update(&md.md5, md5, MD5_HASH_SIZE);
	MD5Update(&md.md5, pass, passlen);
	MD5Update(&md.md5, salt, 8);
	MD5Final(md5, &md.md5);
	memcpy(key + MD5_HASH_SIZE, md5, 24 - MD5_HASH_SIZE);

	memset(md5, 0x0, MD5_HASH_SIZE);
				 ...
				 unsigned char *key, uint32 kLen)
{
	...
	memset(buf, 0x0, SHA1_HASH_SIZE * 2);
