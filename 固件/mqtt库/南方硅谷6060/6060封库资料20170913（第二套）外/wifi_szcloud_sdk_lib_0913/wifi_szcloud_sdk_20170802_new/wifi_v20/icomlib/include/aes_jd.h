...
typedef struct Aes {
    ...
    ALIGN16 U32 reg[AES_BLOCK_SIZE / sizeof(U32)];      /* for CBC mode */
    ALIGN16 U32 tmp[AES_BLOCK_SIZE / sizeof(U32)];      /* same         */
} Aes;
...
int device_aes_encrypt(const U8 * key, int keyLength, 
        const U8 * iv, const U8 *pPlainIn, int plainLength, 
        U8 *pEncOut, int maxOutLen);

int device_aes_decrypt(const U8 * key, int keyLength, const U8 * iv, const U8 *pEncIn, int encLength, U8 *pPlainOut, int maxOutLen);
