...
*/
	...
	for (i = 0; i < 3; i++) {
		psSha1Init(&sha1Ctx.sha1);
		psSha1Update(&sha1Ctx.sha1, salt[i], i + 1);
		psSha1Update(&sha1Ctx.sha1, ssl->sec.premaster, ssl->sec.premasterSize);
		psSha1Update(&sha1Ctx.sha1, ssl->sec.clientRandom, SSL_HS_RANDOM_SIZE);
		psSha1Update(&sha1Ctx.sha1, ssl->sec.serverRandom, SSL_HS_RANDOM_SIZE);
		psSha1Final(&sha1Ctx.sha1, buf);
...
*/
	...
	for (i = 0; i < keyIter; i++) {
		psSha1Init(&sha1Ctx.sha1);
		psSha1Update(&sha1Ctx.sha1, salt[i], i + 1);
		psSha1Update(&sha1Ctx.sha1, masterSecret, secretLen);
		psSha1Update(&sha1Ctx.sha1, serverRandom, SSL_HS_RANDOM_SIZE);
		psSha1Update(&sha1Ctx.sha1, clientRandom, SSL_HS_RANDOM_SIZE);
		psSha1Final(&sha1Ctx.sha1, buf);
...
*/
	if (sender >= 0) {
		psSha1Update(sha1.sha1,
			(sender & SSL_FLAGS_SERVER) ? SENDER_SERVER : SENDER_CLIENT, 4);
	}
	psSha1Update(sha1.sha1, masterSecret, SSL_HS_MASTER_SIZE);
	psSha1Update(sha1.sha1, pad1, 40);
	psSha1Final(sha1.sha1, ihash);

	psSha1Init(&osha1.sha1);
	psSha1Update(&osha1.sha1, masterSecret, SSL_HS_MASTER_SIZE);
	psSha1Update(&osha1.sha1, pad2, 40);
	psSha1Update(&osha1.sha1, ihash, SHA1_HASH_SIZE);
	psSha1Final(&osha1.sha1, out + MD5_HASH_SIZE);
						...
						unsigned char *mac)
{
	...
	psSha1Init(&sha1.sha1);
	psSha1Update(&sha1.sha1, key, SHA1_HASH_SIZE);
	psSha1Update(&sha1.sha1, pad1, 40);
	psSha1Update(&sha1.sha1, seq, 8);
	ihash[0] = type;
	ihash[1] = (len & 0xFF00) >> 8;
	ihash[2] = len & 0xFF;
	psSha1Update(&sha1.sha1, ihash, 3);
	psSha1Update(&sha1.sha1, data, len);
	psSha1Final(&sha1.sha1, ihash);

	psSha1Init(&sha1.sha1);
	psSha1Update(&sha1.sha1, key, SHA1_HASH_SIZE);
	psSha1Update(&sha1.sha1, pad2, 40);
	psSha1Update(&sha1.sha1, ihash, SHA1_HASH_SIZE);
	psSha1Final(&sha1.sha1, mac);
