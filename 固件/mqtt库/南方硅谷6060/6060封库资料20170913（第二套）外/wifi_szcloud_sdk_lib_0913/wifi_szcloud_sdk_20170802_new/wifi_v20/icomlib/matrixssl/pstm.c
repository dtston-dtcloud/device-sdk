...
int32 pstm_mul_d(pstm_int *a, pstm_digit b, pstm_int *c)
{
	...
	w       = 0;
	for (x = 0; x < a->used; x++) {
        tmpw      = (pstm_word)a->dp[x] * (pstm_word)b;
        tmpd     = (pstm_digit)tmpw;
		c->dp[x]  = (pstm_digit)(tmpd + w);
        if(c->dp[x] < (pstm_digit)w && c->dp[x] < tmpd)
        {
            tmpw = tmpw + 0x100000000;
        }
		w         = tmpw >> DIGIT_BIT;
	}
