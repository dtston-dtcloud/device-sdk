...
int getpeerIPv4port(NETSOCKET socket, int *p_port)
{
	struct uip_conn *conn;
...
int tcprecv(NETSOCKET socket, char *data, U16 datalen)
{
	...
	{
        if(datalen >= (uip_datalen() - recvoffset))
            recvlen = uip_datalen() - recvoffset;
        else
            recvlen = datalen;
...
#if ENABLE_UIP_BUFFER
        if((appstate->datalen > 0) && (appstate->sendlen == 0)) 
#else
...
#if ENABLE_UIP_BUFFER
    tmpbuf = (char *)malloc(UIP_BUFFER_SIZE);
    if(tmpbuf == NULL)
        return -1;
#endif
...
int udprecvfrom(NETSOCKET socket, char *data, U16 datalen, uip_ipaddr_t *peeraddr, U16 *peerport)
{
	...
	if(uip_checkudpsocket(socket) == 0)
	{
		...
		if(peeraddr != NULL)
			memcpy(peeraddr, &(rxBUF.RUDPBUF->srcipaddr), sizeof(uip_ipaddr_t));
		if(peerport != NULL)
			*peerport = UIP_HTONS(rxBUF.RUDPBUF->srcport);
		return uip_datalen();
	}
	else
		return -1;
