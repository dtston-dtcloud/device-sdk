...
extern void softap_set_max_sta_num(U8 max_sta_num);
extern void softap_set_encryt_mode(U8 encryt_mode);
extern void softap_set_passphras(U8 *key, U8 keylen);
extern S32 softap_set_channel_setting(U8 channel);
/*---------------------------------------------------------------------------*/
#if 0
...
void softap_dump_dhcp_setting()
{
  S32 loop_i = 0;
    ...
    for( loop_i=0; loop_i < gwifistatus.softap_max_sta_num; loop_i++)
    {
...
{
	#define	D_Host_LocalIp			10
    S32 rlt = 0;
    S32 loop_i = 0;

    uip_set_hostaddr( gw_digit0, gw_digit1, gw_digit2, D_Host_LocalIp); //set our own ip address to uip
    ...
    g_softap_config.dnsaddr.u8[1] = gw_digit1;
    g_softap_config.dnsaddr.u8[2] = gw_digit2;
    g_softap_config.dnsaddr.u8[3] = D_Host_LocalIp;

    //default router: 192.168.0.1
    g_softap_config.default_router.u8[0] = gw_digit0;
    g_softap_config.default_router.u8[1] = gw_digit1;
    g_softap_config.default_router.u8[2] = gw_digit2;
    g_softap_config.default_router.u8[3] = D_Host_LocalIp;
    ...
    for( loop_i=0; loop_i < gwifistatus.softap_max_sta_num; loop_i++)
    {
        ...
        g_softap_lease[loop_i].ipaddr.u8[1] = gw_digit1;
        g_softap_lease[loop_i].ipaddr.u8[2] = gw_digit2;
        g_softap_lease[loop_i].ipaddr.u8[3] = D_Host_LocalIp - 1 + 10 + loop_i;
        g_softap_lease[loop_i].lease_end = 0;  //will be set when allocate_address
    }
    
   
    g_softap_config.leases = &g_softap_lease[0];
    g_softap_config.num_leases = gwifistatus.softap_max_sta_num;
...
/*---------------------------------------------------------------------------*/
S32 softap_init_ex2(U8 gw_digit0,U8 gw_digit1,U8 gw_digit2,U8 max_sta_num,U8 encryt_mode, U8 keylen,U8 *key,U8 channel,U8 beacon_interval)
{
    S32 rlt = 0;

    softap_set_max_sta_num(max_sta_num);
    softap_set_encryt_mode(encryt_mode);
    softap_set_passphras(key, keylen);

    softap_set_channel_setting(channel);

    rlt = softap_init_ex(gw_digit0,gw_digit1,gw_digit2,beacon_interval);
    if(rlt !=0)
    {
        goto exit;
    }
...
/*---------------------------------------------------------------------------*/
S32 softap_init_ex(U8 gw_digit0,U8 gw_digit1,U8 gw_digit2,U8 beacon_interval)
{
    S32 rlt = 0;
    ...
    gwifistatus.softap_beacon_interval = beacon_interval;
...
S32 softap_init()
{
    ...
    U8 setting_encryt_mode = gwifistatus.softap_encryt_mode;
    U8 setting_channel = gwifistatus.softap_channel;
    U8 setting_keylen = gwifistatus.softap_keylen;
    U8 setting_key[64] = {0};
    U8 beacon_interval = CFG_SOFTAP_BEACON_INTERVAL;
    memcpy(setting_key, gwifistatus.softap_key, 64);	

#if !defined(ENABLE_4STA_USE_HW_WPA2)
    //usage1: 1*STA + encryption + channel 1
    rlt = softap_init_ex2( 192, 168, 0, 1, setting_encryt_mode, setting_keylen,setting_key,setting_channel,beacon_interval); //it means to set gateway as 192.168.0.1    
#else
    //usage1: 4*STA + encryption + channel 1
    rlt = softap_init_ex2( 192, 168, 0, 4, setting_encryt_mode, setting_keylen,setting_key,setting_channel,beacon_interval); //it means to set gateway as 192.168.0.1
#endif
    if(rlt != 0)
    {
        printf("<Error>softap_init_ex2 failed!! rlt = %d\n",rlt);
    }
    return rlt;
}
