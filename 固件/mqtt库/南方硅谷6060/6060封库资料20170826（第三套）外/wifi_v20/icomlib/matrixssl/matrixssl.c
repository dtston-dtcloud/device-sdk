...
static int32 matrixSslLoadKeyMaterialbuf(sslKeys_t *keys,
				const unsigned char *certBuf, int32 certLen,
				const unsigned char *privBuf, int32 privLen,
				const char *privPass, const unsigned char *CAbuf, int32 CAlen,
				int32 privKeyType);
...
#ifdef USE_RSA
int32 matrixSslLoadRsaKeysbuf(sslKeys_t *keys,
				const unsigned char *certBuf, int32 certLen,
				const unsigned char *privBuf, int32 privLen,
				const char *privPass, const unsigned char *CAbuf, int32 CAlen)
{
	return matrixSslLoadKeyMaterialbuf(keys, certBuf, certLen,
				privBuf, privLen, privPass, CAbuf, CAlen, PS_RSA);
...
int32 matrixSslLoadRsaCAarray(sslKeys_t *keys, char *CAarray, int32 CAlen)
{
	psPool_t	*pool;
    int32 ret;
	...
	pool = keys->pool;
	ret = psX509ParseCertArray(pool, CAarray, CAlen, &keys->CAcerts, PS_RSA);
    return ret;
}
#endif /* USE_RSA */
...
*/
	...
	if (certBuf) {
		...
		if ((err = psX509ParseCertArray(pool, certBuf, certLen,
						&keys->cert, flags)) < 0) {
			return err;
		}
...
*/
	if (privBuf) {
		...
		if (privKeyType == PS_RSA) {
			if ((err = pkcs1ParsePrivbuf(pool, privBuf, privLen, (char*)privPass,
			            &keys->privKey)) < 0) {
				if (keys->cert) {
					psX509FreeCert(keys->cert);
					keys->cert = NULL;
		...
		if (privKeyType == PS_ECC) {
            //Do not support ECC buf mode for now.
		}
#endif /* USE_ECC */
#else
...
*/
	...
	if (CAbuf) {
		...
		err = psX509ParseCertArray(pool, CAbuf, CAlen,
						&keys->CAcerts, flags);
		if (err >= 0) {
