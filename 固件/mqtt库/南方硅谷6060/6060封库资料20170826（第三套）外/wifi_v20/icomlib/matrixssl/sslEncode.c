...
*/
	if (ssl->flags & SSL_FLAGS_DHE_WITH_RSA) {
		...
		if (ssl->flags & SSL_FLAGS_TLS_1_2) {
			...
			MD5Init(&digestCtx.md5);
			MD5Update(&digestCtx.md5, ssl->sec.clientRandom, SSL_HS_RANDOM_SIZE);
			MD5Update(&digestCtx.md5, ssl->sec.serverRandom, SSL_HS_RANDOM_SIZE);
			MD5Update(&digestCtx.md5, sigStart, (uint32)(c - sigStart));
			MD5Final(hsMsgHash, &digestCtx.md5);
		...
		tlsExtension_t *userExt, sslSessOpts_t *options)
{
	...
	psTraceInfo("<<< Client creating CLIENT_HELLO  message\n");
	*requiredLen = 0;
...
static int32 writeClientKeyExchange(ssl_t *ssl, sslBuf_t *out)
{
	...
	psTraceInfo("<<< Client creating CLIENT_KEY_EXCHANGE message\n");
...
//	printf("<<< Client creating CERTIFICATE_VERIFY  message\n");
	c = out->end;
	end = out->buf + out->size;
