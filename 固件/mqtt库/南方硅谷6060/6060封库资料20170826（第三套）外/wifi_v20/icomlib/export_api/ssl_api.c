...
{
	if (sslctx->hsState == SSL_HS_DONE) {
		if (sslctx->minVer == SSL3_MIN_VER) {
			printf("SSL 3.0 ");
		} else if (sslctx->minVer == TLS_MIN_VER) {
			printf("TLS 1.0 ");
		} else if (sslctx->minVer == TLS_1_1_MIN_VER) {
			printf("TLS 1.1 ");
		} else if (sslctx->minVer == TLS_1_2_MIN_VER) {
			printf("TLS 1.2 ");
		}
		switch (sslctx->cipher->ident) {
			case TLS_RSA_WITH_AES_128_CBC_SHA:
				printf("TLS_RSA_WITH_AES_128_CBC_SHA\n");
				break;
			case TLS_RSA_WITH_AES_256_CBC_SHA:
				printf("TLS_RSA_WITH_AES_256_CBC_SHA\n");
				break;
			case TLS_RSA_WITH_AES_128_CBC_SHA256:
				printf("TLS_RSA_WITH_AES_128_CBC_SHA256\n");
				break;
			case TLS_RSA_WITH_AES_256_CBC_SHA256:
				printf("TLS_RSA_WITH_AES_256_CBC_SHA256\n");
				break;
			default:
				printf("!!!! DEFINE ME %d !!!!\n", sslctx->cipher->ident);
		}
...
sslKeys_t *SSL_loadkeysbybuf(char *cert, int certlen, char *key, int keylen, char *rootCA, int rootCAlen)
{
    ...
    matrixSslNewKeys(&keys, NULL);
    rc = matrixSslLoadRsaKeysbuf(keys, cert, certlen, key, keylen, NULL, rootCA, rootCAlen);
...
sslKeys_t *SSL_loadkeys(char *certname, char *keyname, char *rootCAname)
{
    ...
    matrixSslNewKeys(&keys, NULL);
    rc = matrixSslLoadRsaKeys(keys, certname, keyname, NULL, rootCAname);
...
/*                else if (rc == MATRIXSSL_HANDSHAKE_COMPLETE) {
                    ...
                    if(sslrecd->sslctx->hsState == SSL_HS_DONE)
                    {
                        sslmsg.sslctx = sslrecd->sslctx;
                        sslmsg.status = SSL_SENDACK;
                        process_post_synch(sslrecd->sslctx->callbackproc, PROCESS_EVENT_MSG, &sslmsg);
                    }
                }
			}
...
ssl_t *SSL_connect(struct process *callbackproc, sslKeys_t *keys, uip_ipaddr_t peeraddr, U16 port)
{
    ...
    rc = matrixSslNewClientSession(&sslctx, keys, sid, ciphers, ciphercnt,
...
int SSL_read(ssl_t *sslctx, char *buf, int buflen)
{
    ...
    if((sslrecd->recvlen - sslrecvoffset) < buflen)
        buflen = sslrecd->recvlen - sslrecvoffset;
    
    memcpy(buf, sslrecd->recvbuf + sslrecvoffset, buflen);
    sslrecvoffset += buflen;
