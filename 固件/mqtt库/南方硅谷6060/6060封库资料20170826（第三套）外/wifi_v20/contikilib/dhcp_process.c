	...
	while(1) {
		...
		if (gwifistatus.wifi_connect == 1 && (gwifistatus.status == IEEE80211_CONNECTED || gwifistatus.status == IEEE80211_GETIP)) {
			...
			else if (ev == tcpip_event || ev == PROCESS_EVENT_TIMER) 
			{
				...
				if(getDhcpState() == STATE_CONFIG_RECEIVED && gwifistatus.enableSmartSoftFilter == TRUE)
				{
...
dhcp_configure ()
{
	...
	if (gCabrioConf.dhcp_enable == 0) {	// Fix IP process
		...
		ip_configure ();
		replyAck();
		icomprintf(GROUP_NET,LOG_INFO,"+CONNECT:%s\r\n", "SUCCESS");
		icomprintf(GROUP_ATCMD,LOG_INFO,"AT+WIFICONNECT OK");
	...
	{
		icomprintf(GROUP_NET,LOG_INFO,"+CONNECT:%s\r\n", "SUCCESS");
		icomprintf(GROUP_NET,LOG_INFO,"Enable DHCP Process\n");
