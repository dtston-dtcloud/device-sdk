...
{
    sprintf((char *)gSocketNetPara.remoteMAC, "%02x:%02x:%02x:%02x:%02x:%02x", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);		// ´«ËÍ12¸ö×Ö½Ú, qinjiangwei 2017/5/18
}
/*---------------------------------------------------------------------------*/
...
void checkCmdAndSendData(uip_ipaddr_t peeraddr, U16 peerport)
{
    ...
    if(cmd== DISCOVER) {
        ...
        if(gSocketNetPara.wifiUartServerUp == 1) {
				#ifdef	____D_Disable_iComm_WifiUart____
...
PROCESS_THREAD(tcp_smartlink_process, ev, data)
{
    ...
    while(1) {
        ...
        if(gSocketNetPara.smartSockStatus == SOCKET_CLOSED) {
            ...
            if(gSocketNetPara.gTcpClientSmartSock == -1) {
                smartTcpConnect();
            }
        }
        gSocketNetPara.exitSmartLinkCnt++;
        if(gSocketNetPara.exitSmartLinkCnt > 30) {
            smartTcpDisConnect();
    ...
    gwifistatus.enableSmartSoftFilter = FALSE;
